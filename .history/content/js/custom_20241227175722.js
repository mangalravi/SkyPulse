...
$(document).ready(function () {
  ...
  $(".slider").slick({
    ...
    fade: true,
    autoplaySpeed: 3000,
    pauseOnHover: false,  // Disable pausing autoplay on hover
    pauseOnFocus: false   // Disable pausing autoplay when slider is focused
});

// Prevent click on the slider from changing the slide or resetting autoplay
$(".slider").on("click", function (event) {
    event.preventDefault(); // Prevent any default action from happening on click (e.g., slide change)
    // Do nothing on click, autoplay should continue
});

// Ensure active thumbnail is updated when slide changes
$(".slider").on("beforeChange", function (e, slick, currentSlide, nextSlide) {
    $(".thumbnails .owl-item").removeClass("active");
    $('.thumbnails .owl-item[data-slide="' + nextSlide + '"]').addClass("active");
});

// When a thumbnail is clicked, go to the respective slide
...
// Play/Pause functionality for autoplay control
$(".play-pause").data("paused", false);
$(".play-icon").hide();
$(".pause-icon").show();
$(".play-pause").click(function () {
    var paused = $(this).data("paused"),
        activeThumbnail = $(".thumbnails .owl-item.active");
...
// Hover logic: Autoplay should continue even when hovering over the slider
$(".slider").hover(
    function () {
        // Do nothing; autoplay continues without being interrupted
    },
    function () {
        // If autoplay is not paused, it continues running
        if (!$(".play-pause").data("paused")) {
            $(".slider").slick("slickPlay");
        }
    }
);

// Thumbnail hover logic should not interrupt autoplay
$(".thumbnails .owl-item").hover(
    function () {
        // Do nothing on hover; autoplay continues
    },
    function () {
        // Do nothing on hover out; autoplay continues
    }
);
